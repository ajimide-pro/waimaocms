...
class  NewsContent extends Model
{
    ...
    public static function updateAll($list=[],$lang,$admin_user)
    {
        foreach($list as $l) {
         ...
         //   $l['image_array']=json_encode($l['image_array']);
            ...
            $saveAll =  NewsContent::where($where)->strict(false)->update($l);
        }
        return $saveAll;
    }
    ...
    {
        $delpic=NewsContent::where($where)->field('image')->select()->toArray();
        foreach($delpic as $d)
        {
        ...
        $deleteAll=NewsContent::where($where)->delete();
        return $deleteAll;
    }
