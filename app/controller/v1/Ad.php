...
class Ad extends BaseController
{
    ...
    public function Create()
    {
        ...
        $data['lang']=$this->lang;
        $into = $this->ad->strict(false)->insertGetId($data);
        if ($into) {
    ...
    public function Ad_get_list()
    {
        ...
        $list = $this->ad->where($where)->limit($data['limit'])->page($data['page'])->order($order)->select()->toArray();
        $total=$this->ad->where($where)->count();
        foreach($list as $i=>$info) {
            ...
            $list[$i]['position']= (string)$list[$i]['position'];
        }
    ...
    public function Create_edit()
    {
        ...
        if(count($list)>0)
        {
            ...
            $up=$AdContent->updateAll($list, $this->lang,$this->admin_user);
